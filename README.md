...
Claude Code 的自动补全按**名称总长度**排序——名称越短越靠前。skill-habit 让每个排名的快捷键名称长度**严格递增**，方式是将排名数字重复 N 次：
...
当你的使用习惯改变时，下次 session 启动时快捷键会自动重新分配。输入方式：直接打 `/sh1`，或打 `/sh2`（模糊匹配 `sh22`）后按回车。
...
> **环境要求：** macOS 或 Linux · Python 3.7+ · git
>
...
> **方案 A 遇到问题？** 可以直接使用下方的方案 B，效果完全一致。
...
> 默认安装到 `~/.claude/skills/skill-habit`（目录不存在时自动创建）。如需自定义路径：`SKILL_HABIT_INSTALL_DIR=/your/path curl -sSL ... | bash`

**方案 C — pipx / pip**

> 注意：`skill-habit install` 运行时会自动同步内置技能，功能与方案 A/B 完全一致。
...
Cursor、Codex CLI、Gemini CLI 等适配器正在规划中。贡献方式参见"[参与贡献](#-参与贡献)"章节。
...
> 执行后会询问是否同时删除使用历史和配置数据（`~/.skill-habit`），按需选择即可。
...
| `/skill-habit:server`  | 在浏览器中打开 Web 管理界面    | 修改配置、查看分析图表、管理技能列表；浏览器自动打开，5 分钟无操作后服务器自动退出。重复执行会自动重启服务器（不会多开），默认固定端口 5027，端口冲突时自动改用随机端口               |
...
| `/skill-habit:uninstall`  | 卸载 skill-habit               | 自动识别安装方式，清理 hooks、快捷技能及插件数据；执行时会询问是否同时删除使用历史和配置数据     |
...
> Skills 标签页只显示至少使用过一次的技能，数据来源为 `~/.claude/skills/`、所有已安装插件，以及 `~/.claude/commands/` 下的自定义指令（按命名空间分组展示在页面底部的 **Commands** 区域）。
