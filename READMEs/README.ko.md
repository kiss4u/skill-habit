...
> **요구 사항:** macOS 또는 Linux · Python 3.7+ · git
>
...
> 옵션 A에 문제가 있나요? 옵션 B도 완전히 동일하게 작동합니다 — 대신 사용하세요.
...
> 기본 설치 경로는 `~/.claude/skills/skill-habit`입니다（디렉토리가 없으면 자동 생성）. 경로를 변경하려면: `SKILL_HABIT_INSTALL_DIR=/your/path curl -sSL ... | bash`

**옵션 C — pipx / pip**

> 참고: `skill-habit install` 실행 시 내장 스킬이 자동으로 동기화됩니다 — 옵션 A / B와 기능이 완전히 동일합니다.
...
> 사용 기록과 설정 데이터(`~/.skill-habit`)도 삭제할지 묻습니다 — 필요에 따라 선택하세요.
...
| `/skill-habit:server`  | 브라우저에서 웹 관리 UI 열기        | 설정 변경, 분석 조회, 스킬 관리; 브라우저 자동 열림, 5분 비활성 후 서버 자동 종료. 다시 실행하면 서버를 재시작합니다 (창 중복 없음); 기본 포트 5027 고정, 충돌 시 랜덤 포트 사용     |
...
| `/skill-habit:uninstall`  | skill-habit 제거 | 설치 방법을 자동 감지하여 훅, 단축키 스킬, 플러그인 데이터를 정리; 사용 기록과 설정 데이터도 삭제할지 묻습니다 |
...
> 스킬 탭에는 한 번 이상 사용한 스킬만 표시됩니다. 스킬은 `~/.claude/skills/`, 모든 설치된 플러그인, 그리고 `~/.claude/commands/` 아래의 사용자 정의 명령어에서 가져옵니다（페이지 하단의 **Commands** 섹션에 네임스페이스별로 그룹화되어 표시）.
...
| 자신을 통계에서 제외  | 활성화 시 `skill-habit:*` 명령 호출이 통계에 포함되지 않음 (기본값: 활성화)          |
